            ...
            for page in range(total_pages):
                ...
                for i in range(start_idx, end_idx):
                    ...
                    try:
                        ...
                        # 计算文字位置（以坐标点为中心）
                        text1_x = x - (text1_width * x_stretch) / 2
                        text1_y = y - (font_size * y_stretch) - spacing
                        
                        text2_x = x - (text2_width * x_stretch) / 2
                        text2_y = y + spacing
                        
                        # 绘制文字（支持拉伸变形）
                        if x_stretch != 1.0 or y_stretch != 1.0:
                            # 创建临时图像来绘制文字并进行拉伸
                            for text, pos_x, pos_y in [(text1, text1_x, text1_y), (text2, text2_x, text2_y)]:
                                # 创建足够大的临时图像
                                temp_width = int(draw.textlength(text, font=font) * 1.2)
                                ...
                                # 将变形后的文字粘贴到主图像
                                image.paste(transformed_img, (int(pos_x), int(pos_y)), transformed_img)
                        else:
                            # 正常绘制文字
